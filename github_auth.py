...
class GitHubManager:
    ...
    def download_file(self, owner, repo, file_path, save_path):
        """
        下载 GitHub 仓库文件（支持私有仓库），封装 GitHubAuth 的下载接口。
        """
        return self.github_auth.download_file(owner, repo, file_path, save_path)

    # —— 在 GitHubManager 中新增下载方法 —— #
    # 假设你的 github_auth.GitHubManager 已有 session（requests.Session）
    def download_file_with_progress(self, owner, repo, file_path, save_path, progress_callback=None):
        """
        使用 GitHub API 流式下载文件，并回调进度
        """
        url = f"https://raw.githubusercontent.com/{owner}/{repo}/main/{file_path}"
        headers = {}
        token = getattr(self, "token", None)  # 如果你存储了 token
        if token:
    ...
    def setup_github_app(self):
        ...
        class SetupWindow(tk.Toplevel):
            ...
            def setup_ui(self):
                ...
                self.geometry("600x720")  # 将高度从680改为720
                ...
                style.configure('Step.TLabelframe', padding=10, font=("Microsoft YaHei UI", 10))
                style.configure('Info.TLabel', font=("Microsoft YaHei UI", 10))
                style.configure('Action.TButton', padding=5, font=("Microsoft YaHei UI", 10))
                ...
                ttk.Label(step1_frame, text="1. 在新窗口中登录GitHub并注册应用").pack(side=tk.LEFT, padx=10, pady=5)
                url_frame = ttk.Frame(step1_frame)
                url_frame.pack(fill=tk.X, padx=10, pady=5)
                
                url = "https://github.com/settings/applications/new"
                ttk.Label(url_frame, text="请点击右边按钮        ").pack(side=tk.LEFT)
                ttk.Button(url_frame, text="注册GitHub应用", 
                          command=lambda: webbrowser.open(url)).pack(side=tk.LEFT, padx=5)
                
                # 第2步
                step2_frame = ttk.LabelFrame(main_frame, text="第2步：填写应用信息")
                step2_frame.pack(fill=tk.X, pady=(0, 10))
                
                ttk.Label(step2_frame, text="在打开的GitHub页面中填写以下信息：", 
                         font=("Microsoft YaHei UI", 10, "bold")).pack(padx=10, pady=(5,0))
...
4. 应用描述（可选）：
   字体管理器 OAuth 应用
                ...
                ttk.Button(copy_frame, text="复制应用名称", 
                          command=lambda: self.copy_to_clipboard("Font Manager")).pack(side=tk.LEFT, padx=5)
                ttk.Button(copy_frame, text="复制应用描述", 
                          command=lambda: self.copy_to_clipboard("字体管理器 OAuth 应用")).pack(side=tk.LEFT, padx=5)
                ttk.Button(copy_frame, text="复制 Homepage URL", 
                          command=lambda: self.copy_to_clipboard("http://localhost:9826")).pack(side=tk.LEFT, padx=5)
                ttk.Button(copy_frame, text="复制 Callback URL", 
                          command=lambda: self.copy_to_clipboard("http://localhost:9826/callback")).pack(side=tk.LEFT, padx=5)
                
                ttk.Label(step2_frame, text="完成注册后，您将获得 Client ID 和 Client Secret", 
                         font=("Microsoft YaHei UI", 10, "italic")).pack(padx=10, pady=(0,5))
    ...
    def _get_github_token(self):
        ...
        auth_url = (
            ...
            f"&scope=repo"
            f"&state={state}"
            f"&redirect_uri=http://localhost:9826/callback"
        )
